...
9. **Full-width → half-width auto-correction.** When CJK users forget to switch input methods and type fullwidth ASCII (e.g., `－－ｖｅｒｂｏｓｅ` instead of `--verbose`, or `＝` instead of `=`), ArgMojo auto-detects and corrects these characters with a coloured warning. Fullwidth spaces (`U+3000`) embedded in a token cause it to be split into multiple arguments. Only option tokens (starting with `-` after correction) are corrected; positional values are left unchanged. Disabled via `disable_fullwidth_correction()`. Custom Unicode whitespace characters can be registered via `whitespace_characters()` (PR #15).
...
- Add `tests/test_fullwidth.mojo` with 20 tests covering full-width → half-width auto-correction, including utility functions, fullwidth flags, equals syntax, embedded fullwidth spaces, opt-out, choices validation, merged short flags, subcommand dispatch, parse_known_arguments, and custom whitespace characters (PR #15).
